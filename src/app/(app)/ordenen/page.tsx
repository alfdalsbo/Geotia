...
export default async function GeoticOrderPage() {
  ...
  return (
    <div className="space-y-7">
      ...
      <div id="ordensstigen">
        <Section title="Veien opp gjennom Geotia" eyebrow="Den synlige rangstigen">
          <p className="mb-5 max-w-3xl text-sm leading-6 text-[#60553f]">
      ...
      <div id="protokollen">
        <Section
          title="Geotenes nåværende rang"
          eyebrow="Rikets ordensprotokoll"
          action={
            <Link
              href="/stilling"
              className="inline-flex h-10 items-center justify-center gap-2 rounded border border-[#c49a3c]/55 bg-[#fff7e6] px-3 text-sm font-semibold text-[#062b40] transition hover:border-[#fff7e6]"
            >
              Se poenggrunnlaget
              <ArrowRight className="h-4 w-4" aria-hidden="true" />
            </Link>
        ...
        <Section title="Ritualene" eyebrow="Løfter som virker fordi de nesten ikke gjør det">
          <div className="grid gap-3">
            {geoticOrderRanks.slice(0, 6).map((rank) => (
              <div key={rank.id} className="rounded border border-[#d8c48c] bg-[#fff7e6] p-3 shadow-sm">
        ...
        <Section title="Partiprøvene" eyebrow="Aspirantens syv dører">
          <div className="grid gap-3 md:grid-cols-2">
            {partyTrials.map((trial) => (
              <div key={trial} className="rounded border border-[#d8c48c] bg-[#fff7e6] p-3 text-sm leading-6 text-[#4f412b] shadow-sm">
                {trial}
              </div>
            ))}
          </div>
...
function PersonalPathCard({ row }: { row: OrderRow }) {
  ...
  return (
    <div className="rounded border border-[#c49a3c]/55 bg-[#fff7e6] p-5 shadow-sm">
